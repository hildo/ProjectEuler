...
object Problem011 {
  val grid = Array(
    ...
    Array(52, 70, 95, 23, 4, 60, 11, 42, 69, 24, 68, 56, 01, 32, 56, 71, 37, 2, 36, 91),
  ...
  def up(x: Int, y: Int): Int = {
    if (x >= 3)
      List(grid.apply(x).apply(y), grid.apply(x - 1).apply(y), grid.apply(x - 2).apply(y), grid.apply(x - 3).apply(y)).product
    else
      0
  }
  ...
  def left(x: Int, y: Int): Int = {
    if (y >= 3)
      List(grid.apply(x).apply(y), grid.apply(x).apply(y - 1), grid.apply(x).apply(y - 2), grid.apply(x).apply(y - 3)).product
    else
      0
  }
  ...
  def diagur(x: Int, y: Int): Int = {
    if ((x >= 3) && (y <= 16))
      List(grid.apply(x).apply(y), grid.apply(x - 1).apply(y + 1), grid.apply(x - 2).apply(y + 2), grid.apply(x - 3).apply(y + 3)).product
    else
      0
  }

  def diagdr(x: Int, y: Int): Int = {
    if ((x <= 16) && (y <= 16))
      List(grid.apply(x).apply(y), grid.apply(x + 1).apply(y + 1), grid.apply(x + 2).apply(y + 2), grid.apply(x + 3).apply(y + 3)).product
    else
      0
  ...
  def maxAt(x: Int, y: Int): Int = {
    Set(up(x,y), down(x,y), left(x,y), right(x,y), diagdl(x, y), diagdr(x, y), diagul(x, y), diagur(x, y)).max
  }

  def buildSums: Set[Int] = {
    val sums = for {
      x <- 0 to 19
      y <- 0 to 19
    } yield maxAt(x,y)
